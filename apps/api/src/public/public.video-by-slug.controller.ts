...
export class PublicVideoBySlugController {
  ...
  async get(@Param('slug') slug: string, @Query('locale') locale: string = 'en') {
    ...
    const t = v.translations.find((x) => x.locale === locale) ?? v.translations.find((x) => x.locale === 'en') ?? null;
    ...
    if (v.uploaderVisible && v.uploaderId) {
      const profile = await this.prisma.creatorProfile.findUnique({ where: { userId: v.uploaderId } });
    ...
    return {
      ...
      uploader,
      channels: v.channels.map((c) => ({ slug: c.channel.slug, name: c.channel.name })),
